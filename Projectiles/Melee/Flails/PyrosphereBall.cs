...
{
    public class PyrosphereBall : ModProjectile
	{
        private const string ChainTexturePath = "MultidimensionMod/Projectiles/Melee/Flails/PyrosphereChain"; // The folder path to the flail chain sprite
        ...
        public override void AI()
        {
            ...
            float maxRetractSpeed = 8f; // The max speed the projectile will have while retracting
            ...
            float maxForcedRetractSpeed = 12f; // The max speed the projectile will have while being forced to retract
            float unusedRetractAcceleration = 1f;
            float unusedMaxRetractSpeed = 14f;
            int unusedChainLength = 60;
            int defaultHitCooldown = 15; // How often your flail hits when resting on the ground, or retracting
            int spinHitCooldown = 20; // How often your flail hits when spinning
            int movingHitCooldown = 15; // How often your flail hits when moving
                ...
                case AIState.Spinning:
                    {
                        ...
                        // This line creates a unit vector that is constantly rotated around the player. 8f controls how fast the projectile visually spins around the player
                        Vector2 offsetFromPlayer = new Vector2(player.direction).RotatedBy((float)Math.PI * 8f * (SpinningStateTimer / 60f) * player.direction);
                        ...
                        if (shouldSwitchToRetracting)
                        {
                            if (!drowning)
                            {
                                if (Main.myPlayer == Projectile.owner)
                                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.velocity, ModContent.ProjectileType<PyrosphereUnchained>(), Projectile.damage / 2, Projectile.knockBack, Main.myPlayer);
                                Projectile.Kill();
                            }
                            else
                            {
			...
			*/
            ...
            if (Main.rand.NextBool(dustRate) && !drowning)
