...
namespace MultidimensionMod.Biomes
{
    public class SporeHaze
    {
        ...
        public void Update(Texture2D texture)
        {
			...
			bool inBiome = player.InModBiome<ShroomForest>();
			...
			if(!backgroundFog)
			{
				dayTimeOpacity = Main.dayTime ? 0f : 0.1f;		
			}
			else
			{
				dayTimeOpacity = Main.dayTime ? 0f : 0.1f;
        ...
        public void Draw(Texture2D texture, bool dir, Color defaultColor, bool setSB = false)
        {
			...
			Color fogColor = GetAlpha(new Color(20,0,93), fadeOpacity * dayTimeOpacity);
