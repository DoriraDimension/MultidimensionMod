...
namespace MultidimensionMod.UI
{
    public class TitleCard : UIState // Code by Seraph
    {
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            Texture2D lineTexture = ModContent.Request<Texture2D>("MultidimensionMod/UI/TitleLine").Value;
            Texture2D leftEndTexture = ModContent.Request<Texture2D>("MultidimensionMod/UI/TitleLineEndLeft").Value;
            Texture2D rightEndTexture = ModContent.Request<Texture2D>("MultidimensionMod/UI/TitleLineEndRight").Value;
            ...
            for (int i = centerX - (totalLength / 2) + (totalLength / 2 % 38); i <= centerX + (totalLength / 2) - (totalLength / 2 % 38); i += 38)
            {
                ...
                spriteBatch.Draw(lineTexture, lineDrawPos, lineTexture.Bounds, drawColor * opacity, 0, MDHelper.GetOrigin(lineTexture), FontScale, SpriteEffects.None, 0);
            }
            //draw line endings
            Vector2 leftEndPos = new(centerX - (totalLength / 2) + (totalLength / 2 % 38) - 23, centerY);
            spriteBatch.Draw(leftEndTexture, leftEndPos, leftEndTexture.Bounds, drawColor * opacity, 0, MDHelper.GetOrigin(leftEndTexture), FontScale, SpriteEffects.None, 0);

            Vector2 rightEndPos = new(centerX + (totalLength / 2) - (totalLength / 2 % 38) + 23, centerY);
            spriteBatch.Draw(rightEndTexture, rightEndPos, rightEndTexture.Bounds, drawColor * opacity, 0, MDHelper.GetOrigin(rightEndTexture), FontScale, SpriteEffects.None, 0);
